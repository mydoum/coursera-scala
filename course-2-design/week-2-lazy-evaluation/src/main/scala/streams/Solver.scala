...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] =
    b.legalNeighbors.map(elem => (elem._1, history:::List(elem._2))).toStream
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors.filter(elem => !explored.contains(elem._1))
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val result = for (
          state <- initial;
          neighbors <- newNeighborsOnly(neighborsWithHistory(state._1, state._2), explored)
      ) yield neighbors

      initial ++ from(result, explored ++ result.map(elem => elem._1))
    }
  }
