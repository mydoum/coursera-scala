...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val res = points.groupBy(p => findClosest(p, means))
    res ++ means.filter(mean => !res.contains(mean)).map(mean => (mean, GenSeq[Point]())).toMap
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zip(newMeans).forall(v => v._1.squareDistance(v._2) <= eta)
  }
